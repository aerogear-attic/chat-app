...
export function useOfflineMutation<TData, TVariables = OperationVariables>(
  ...
  const client = useApolloClient(baseOptions.client) as ApolloOfflineClient;
  ...
  const onOfflineError = (error: any, mutationVariables: TVariables | undefined, mutationId: number): Promise<ExecutionResult<TData>> => {
    return new Promise((resolve, reject) => {
      if (isMostRecentMutation(mutationId)) {
        ...
        offlineError.watchOfflineChange().then((result: any) => {
  ...
  const onMutationCompleted = (
    ...
    console.log('onMutationCompleted', data, errors)
    ...
    (mutateOptions: MutationHookOptions<TData, TVariables> = {}) => {
      return new Promise<FetchResult<TData>>((resolve, reject) => {
        ...
        client
          ...
          .then(response => {
            console.log('response from offlineMutation')
